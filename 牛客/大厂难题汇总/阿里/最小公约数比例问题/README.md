...
这道题其实可以看成最大公约数，比如1000 500的公约数是1，就是1000 500找出两个unit最小单位，这里最小单位是Math.min(A/a,B/b)

最大公约数用来把a/b化简，避免出现6/3这种情况，要变成2/1才能找到(是化简a b不是A B)
...
GCD就是找最大公约数，直到把一个变成0

这里顺便科普下最小公倍数

m n 的最小公倍数是 (m X n)/最大公约数
